...
import {
  ...
  MessageSquare, Users, ShieldCheck, ChevronRight, Bot
} from 'lucide-react'
...
const moreFeatures = [
  { icon: Bot,           label: 'AI Safety Assistant', path: '/assistant',   desc: 'Ask safety questions or get app help', isNew: true },
      ...
      <div className="max-w-4xl mx-auto space-y-8 animate-slide-up">
        ...
        <section>
          ...
          <div className="card divide-y dark:divide-surface-border divide-slate-100">
            {moreFeatures.map(({ icon: Icon, label, path, desc, isNew }) => (
                <button
                    key={path}
                    onClick={() => navigate(path)}
                    className="flex items-center gap-4 p-4 w-full text-left
                dark:hover:bg-surface-muted hover:bg-slate-50
                transition-colors duration-150 first:rounded-t-3xl last:rounded-b-3xl"
                >
                ...
                flex items-center justify-center flex-shrink-0">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold dark:text-slate-200 text-slate-800 flex items-center gap-2">
                      {label}
