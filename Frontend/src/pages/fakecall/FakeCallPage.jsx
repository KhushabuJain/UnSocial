...
import { Plus, Pencil, Trash2, Phone, PhoneOff, Star, Zap, Volume2 } from 'lucide-react'
import { getTemplates, createTemplate, updateTemplate, deleteTemplate, setDefaultTemplate, triggerFakeCall } from '../../api/fakeCall.js'
import { RINGTONES, playRingtone, stopRingtone, previewRingtone, unlockAudio } from '../../utils/ringtones.js'
...
const EMPTY = { callerName: '', callerPhone: '', delaySeconds: 5, ringtone: 'classic', makeDefault: false }
...
export default function FakeCallPage() {
  ...
  useEffect(() => { load() }, [])
  useEffect(() => () => stopRingtone(), []) // stop any ringing sound on unmount

  const showToast = (m) => { setToast(m); setTimeout(() => setToast(''), 3000) }
  const set = (f) => (e) => setForm(p => ({ ...p, [f]: e.target.type === 'checkbox' ? e.target.checked : e.target.value }))

  const openAdd  = () => { setEditing(null); setForm(EMPTY); setError(''); setModal(true) }
  const openEdit = (t) => { setEditing(t); setForm({ callerName: t.callerName, callerPhone: t.callerPhone || '', delaySeconds: t.delaySeconds, ringtone: t.ringtone || 'classic', makeDefault: t.default }); setError(''); setModal(true) }
  ...
  const handleTrigger = async (templateId = null) => {
    unlockAudio() // must happen synchronously within the click for autoplay policies
    try {
      const res = await triggerFakeCall({ templateId })
      const { callerName, callerPhone, delaySeconds, ringtone } = res.data.data
      setCallDetails({ callerName, callerPhone, ringtone })
  ...
  // Start/stop the actual ring sound exactly while the call screen is showing —
  // the countdown beforehand stays silent, mirroring a real incoming call.
  useEffect(() => {
    if (showCall) playRingtone(callDetails?.ringtone || 'classic')
    else stopRingtone()
                  ...
                  <div key={t.id} className="flex items-center gap-3 p-4 group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs dark:text-slate-500 text-slate-400 mt-0.5">
                        {t.callerPhone || 'No number'} · Rings in {t.delaySeconds}s · {RINGTONES.find(r => r.id === t.ringtone)?.label || 'Classic Ring'}
                      </p>
                    </div>
            ...
            <Modal title={editing ? 'Edit Template' : 'New Call Template'} onClose={() => setModal(false)}>
              ...
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium dark:text-slate-300 text-slate-700">Ringtone</label>
                  <div className="flex gap-2">
                    <select value={form.ringtone} onChange={set('ringtone')} className="input-base">
                      {RINGTONES.map(r => <option key={r.id} value={r.id}>{r.label}</option>)}
                    </select>
                    <button type="button" onClick={() => previewRingtone(form.ringtone)}
                            title="Preview ringtone"
                            className="flex-shrink-0 px-3.5 rounded-2xl border border-slate-200 dark:border-surface-border
                    hover:bg-slate-50 dark:hover:bg-surface-muted text-brand-600 dark:text-brand-400 transition-colors">
                      <Volume2 className="w-4 h-4" />
                    </button>
                  </div>
                  <p className="text-xs dark:text-slate-500 text-slate-400">This plays while the fake call is ringing</p>
                </div>
